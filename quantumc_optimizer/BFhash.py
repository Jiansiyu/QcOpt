...
class findsolution(object):
    ...
    def _getNextSwap(self, firstLayout):
        ...
        for physical_pair in self.device_connection:
            ...
            next_layout[physical_pair[0]], next_layout[physical_pair[1]] = next_layout[physical_pair[1]], next_layout[
                physical_pair[0]]
            next_level_candidate.append(hash(tuple(next_layout)))
        return {hash(firstLayout): next_level_candidate}
    ...
    def _getNextSwapDict(self):
        ...
        threadpool.close()
    #
    def _getHashMap(self):
        '''
        :return:
        '''
        all_layout = itertools.permutations(range(self.n_qubit))
        self.hashmap = {hash(item): item for item in all_layout}
    ...
    def updateTarget(self, target, series_layout):
        ...
        '''
        last_layout_hash = series_layout[-1]
        last_layout = self.hashmap[last_layout_hash]
        ...
        if not new_target:
            ...
            next_level = []
            next_layout_candidates_hash = self.next_swap[last_layout_hash]
            for next_layout_candidate_hash in next_layout_candidates_hash:
                next_series_layout = list(series_layout)
                if next_layout_candidate_hash not in next_series_layout:
                    next_series_layout.append(next_layout_candidate_hash)
    ...
    def solver(self):
        initial_layout = tuple( (hash(tuple(range(self.n_qubit)) ),))
        result = self.findLayoutSolution(self.target_circuit, initial_layout)
        return result

if __name__ == '__main__':
    hardware = quantumCircuit.ibmq_circuit('line_6')
